  ...
  return (
    <main className="flex gap-8 flex-col lg:flex-row py-8 px-4">
      ...
      <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="w-full max-w-150 mx-auto h-fit bg-white-400 flex flex-col gap-8 p-4 rounded-4xl shadow-neu-soft">
          ...
          {result.status && result.status === "success" ? <div className="flex gap-2"><div className="py-2 px-1 rounded-full bg-green-300" /><h1>Успешно</h1></div> : <div className="flex gap-2"><div className="py-2 px-1 rounded-full bg-red-300" /><h1>Есть проблемы</h1></div>}
          <ul className="p-4 rounded-2xl bg-white-300">
      ...
      {isError && <div className="w-full h-fit bg-red-400 flex items-center justify-center p-4 rounded-4xl shadow-neu-soft">Что-то пошло не так. Попробуйте позже</div>}
    </main>
  )
}
